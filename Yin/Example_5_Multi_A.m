% This file is for example 5 of A-optimal using Multiplicative algorithm
%
% minimize trace((G2(w)-t*g1(w)*g1(w)^T)^{-1}), sum(w)=1, w>=0 
...
while valid == false
    ...
    for i=1:N
        x=h1(:,i);
        b(i)= (1-t)*x'*v*v*x+t*(x-g)'*v*v*(x-g);
    end
    for i=1:N
        valid = b(i)<trace(v)+1e-10;
        if(valid == false)
            break;
        end
    end
...
for i=1:N
    a(i) = MMPhi_A(G,g,t,h1,i)-trace(inv(G-t*(g*g')));
end
error=max(a);
designpoints=N;
ratio=t;
method='multiplicative_A';
