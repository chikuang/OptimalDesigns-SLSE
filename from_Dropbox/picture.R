...
q = 2;  t1=0.5; t2=0.6; t3 = 0.5; t4=0.6;
...
t1=0.5; t2=0.6; t3 = 0.5; t4=0.6;
## Start Graph
x=seq(from= - 1,to= 1,by=0.001)
par(mfrow=c(2,2))
# dA, t=0.1
plot(x, sapply(x, function(x) d_t(x,t=t1,type=1)),col="black",xlim=c(-1.10,1.10),
     ylim=c(-1.5,0.5),ylab=expression('d'["A"]),type="l",main="A-optimality with t = 0.5",cex.lab=1.5,cex.main=1.5,
     lwd= 2.5);
  abline(h=0,col="red",lty="longdash",lwd= 2.5);
# dA, t=0.6
plot(x, sapply(x, function(x) d_t(x,t=t2,type=1)),col="black",xlim=c(-1.10,1.10),
     ylim=c(-1.5,0.5),ylab=expression('d'["A"]),type="l",main="A-optimality with t = 0.6",cex.lab=1.5,cex.main=1.5,
     lwd= 2.5);
  abline(h=0,col="red",lty="longdash",lwd= 2.5);

# dD, t = 0.2
plot(x, sapply(x, function(x) d_t(x,t=t3,type=2)),col="black",xlim=c(-1.10,1.10),
     ylim=c(-1.5,0.5),ylab=expression('d'["D"]),type="l",main="D-optimality with t = 0.5",cex.lab=1.5,cex.main=1.5,
     lwd= 2.5);
  abline(h=0,col="red",lty="longdash",lwd= 2.5);

# dD, t = 0.6
plot(x, sapply(x, function(x) d_t(x,t=t4,type=2)),col="black",xlim=c(-1.10,1.10),
     ylim=c(-1.5,0.5),ylab=expression('d'["D"]),type="l",main="D-optimality with t = 0.6",cex.lab=1.5,cex.main=1.5,
     lwd= 2.5); 
  abline(h=0,col="red",lty="longdash",lwd= 2.5);
